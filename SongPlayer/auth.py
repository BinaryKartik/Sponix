...
def get_song(song):
    try:
        ...
        date =name["tracks"]['items'][0]['album']['release_date']
        pop =name["tracks"]['items'][0]['popularity']
    ...
    'extract_flat': True,}
        ...
        return sng, art, playurl, img, duration, date, pop
...
def songs():
    ...
    if request.method == 'POST':
        ...
        if current_user.is_authenticated:
            ...
            try:
                name, publish, playurl, img, duration, date, pop = get_song(song)
                # lcs2, lcs = rclyrics(name)
                lcs=False
                if lcs:
                  for m in lcs2:
...
def rclyrics(s):
    try:
        ...
        if (0.55 < m) or (s.lower() in hi.lower()):
            ...
            soup = BeautifulSoup(html, features="html.parser")
            data = soup.find(class_='container_single').find("").text
            backup = data
            lrc = []
            while data.find('[') != -1:
...
def songs_find(song_name):
    ...
    try:
        name, publish, playurl, img, duration, date, pop = get_song(song_name)
        return jsonify({
            "song": song,
            "name": name,
            "publish": publish,
